...
function MetadataCleanupDialogSession({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="flex max-h-[min(44rem,calc(100svh-2rem))] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl"
        onCloseAutoFocus={
          returnFocusTarget
            ? (event) => {
      ...
      >
        <DialogHeader className="shrink-0 px-6 pt-6 pb-4">
          ...
          <DialogDescription>
            {albumTitle
              ? `review suggested title changes for ${albumTitle}. keep the changes you like, deselect the ones you don't`
              : "we found some noise in your track titles. keep the changes you like, deselect the ones you don't"}
          </DialogDescription>
        </DialogHeader>
