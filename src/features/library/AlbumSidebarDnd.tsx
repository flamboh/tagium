...
export function SortableTrackRow({
  ...
  // Mount-only: the parent flips animateEnter off on its next render, and that must not cancel
  // an entrance that is still playing.
  const enterOnMount = useRef(animateEnter);
  useLayoutEffect(() => {
    if (!enterOnMount.current || !outerRef.current) return;
    return animateRowEnter(outerRef.current, innerRef.current);
  }, []);
  ...
  return (
    ...
    >
      <Button
        ...
        variant="ghost"
        className={cn(
          "justify-start h-auto py-2.5 px-4 pr-12 w-full text-left font-normal rounded-none hover:bg-accent/30 active:scale-100 [@media(pointer:coarse)]:min-h-11",
