...
| Performance gates                                                    | `docs/generated/metadata-benchmark.json`: 15.12x large scan throughput, 99.85% deterministic peak-allocation reduction, 96.86% less small-file copied work, and 0.65 ms scheduling p95; every primary asserted gate passed. The unstable, explicitly non-gating small-file wall-clock sample regressed 39.10% and remains reported. |
...
- `bun run benchmark:metadata`: every primary performance/resource gate passed; the report retains
  the failed non-gating best-effort small-file wall-clock indicator.
