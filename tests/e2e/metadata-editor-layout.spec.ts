...
const readEditorLayout = async (page: Page) =>
  page.locator("[data-editor-form-area]").evaluate((formArea) => {
    ...
    const area = formArea as unknown as BrowserElement;
    const summary = browser.document.querySelector("[data-track-file-summary]");
    const textarea = browser.document.querySelector("#track-comment");
    const error = browser.document.querySelector("#track-disc-number-error");
...
for (const viewport of [
  ...
  test(`keeps metadata mode layout stable and lets fields grow at ${viewport.name} size`, async ({
    ...
    await comment.evaluate((element) => {
      ...
      textarea.style.height = `${textarea.getBoundingClientRect().height + 96}px`;
    });
    ...
    await comment.evaluate((element) => {
      (element as unknown as { style: { height: string } }).style.height = "";
    });
